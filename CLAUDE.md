...
- `eslint-plugin-react-hooks` and `@next/eslint-plugin-next` are explicit `devDependencies` even though `eslint-config-next` already depends on them — `next build`'s internal lint step can't resolve them otherwise under pnpm's strict isolation. Don't remove them as "duplicates."
